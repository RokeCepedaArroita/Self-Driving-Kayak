...
float alpha_fading_memory = 1.1; // memory fading parameter - higher values result in less lag and higher noise, 1.1 is about right
...
float calmest[]  = {4.0, 13.5, 0.79, 1.1, 0.45}; // P, D, I, sigma_compass, sigma_gyro
float calm[]     = {2.0,  7.6, 0.35, 1.1, 0.45}; // P, D, I, sigma_compass, sigma_gyro
float rough[]    = {1.0,  5.0, 0.13, 1.1, 0.45}; // P, D, I, sigma_compass, sigma_gyro
float roughest[] = {2.25,   0, 0.18, 1.1, 0.45}; // P, D, I, sigma_compass, sigma_gyro
...
void loop() {
  ...
  // Serial.print(total_time_ms);
  // Serial.print(" ");
  // Serial.print(filtered_angular_velocity);
  // Serial.print(" ");
  ...
  if (onoff == 'N') {
    ...
    // Twin engine mode: give power to both engines, either with or without autopilot
      ...
      // AUTOPILOT ENGAGED
        ...
        smoothing_time = 0.1;
        derivative_smoothing_time = 1.0;

        if (autopilot_mode == 1) { // Calmest
          kp = calmest[0];
          kd = calmest[1];
          ki = calmest[2];
          sigma_compass = calmest[3];
          sigma_gyro = calmest[4];
        }

        else if (autopilot_mode == 2) { // Calm
          kp = calm[0];
          kd = calm[1];
          ki = calm[2];
          sigma_compass = calm[3];
          sigma_gyro = calm[4];
        }

        else if (autopilot_mode == 3) { // Rough
          kp = rough[0];
          kd = rough[1];
          ki = rough[2];
          sigma_compass = rough[3];
          sigma_gyro = rough[4];
        }

        else if (autopilot_mode == 4) { // Roughest
          kp = roughest[0];
          kd = roughest[1];
          ki = roughest[2];
          sigma_compass = roughest[3];
          sigma_gyro = roughest[4];
        }
